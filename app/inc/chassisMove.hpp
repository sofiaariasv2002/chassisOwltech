...
class chassisMove {
   private:
    ...
    BaseType_t xQueueSend(QueueHandle_t queue, const TDB* data, TickType_t xTicksToWait);

    BaseType_t xQueueReceive(QueueHandle_t queue, TDB* data, TickType_t xTicksToWait);

   public:
    chassisMove(IntfMotor* leftFrontMotor, IntfMotor* rightFrontMotor, IntfMotor* leftBackMotor,
                IntfMotor* rightBackMotor, float maxMotorSpeed_rpm = MAX_MOTOR_SPEED);
    ~chassisMove();
